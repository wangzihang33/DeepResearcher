...
5. 检索查询应优先命中官方文档、官方仓库、论文、标准、政府或企业一手资料，减少仅依赖聚合博客和社区观点。
</GOAL>
...
<FORMAT>
完成必要的 `note` 工具调用后，最终回复必须只包含下面的 JSON 对象。
不要输出解释、Markdown 表格、代码围栏或 `[TOOL_CALL:...]`，并且必须在 `tasks` 中完整保留全部 3~5 个任务：
{{
  "tasks": [
    {{
      "title": "任务名称（10字内，突出重点）",
      "intent": "任务要解决的核心问题，用1-2句描述",
      "query": "建议使用的检索关键词，优先包含 official documentation、official repository、paper 或 primary source 等一手来源约束"
    }}
  ]
}}
</FORMAT>
...
- 以小节标题开头："任务总结"；
- 关键发现使用有序或无序列表表达；
- 每条关键发现必须是可被来源验证的完整陈述，避免无依据的精确数字、唯一性判断和绝对化排名；
- 优先采用官方文档、论文、标准、政府或企业一手资料；社区文章只能作为补充观点；
- 若任务无有效结果，输出"暂无可用信息"。
