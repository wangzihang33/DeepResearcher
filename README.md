...
本文档记录项目竞争力优化路线与当前阶段进度。Phase 1 的 LangGraph 多 Agent 编排和 Phase 2 的 Evidence Graph 引用校验已经完成 V1，下一步进入深度研究质量评测。
...
- 缺少 baseline 与当前多 Agent 研究图的统一对比报告，尚未用固定指标证明改造收益。
- Citation Verifier V1 采用确定性词项匹配，需要通过评测集验证阈值和校验准确性，再决定是否升级为语义或 LLM 校验。
...
3. Phase 3 下一步：构建深度研究评测集，用量化结果验证多 Agent 编排和证据校验的实际收益。
